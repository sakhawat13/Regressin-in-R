...
> ### Simple regression with lm (dependant - Cohessiveness , Independant - number of establishment)


```R
cohessive.fit = lm(clustering_zip~ESTAB, data=dataset)
summary(cohessive.fit)
```


    
    Call:
    lm(formula = clustering_zip ~ ESTAB, data = dataset)
    
    Residuals:
          Min        1Q    Median        3Q       Max 
    -0.050274 -0.011778 -0.001926  0.009298  0.307261 
    
    Coefficients:
                  Estimate Std. Error t value Pr(>|t|)    
    (Intercept)  1.117e-01  1.487e-04  750.92   <2e-16 ***
    ESTAB       -9.050e-06  1.312e-07  -68.96   <2e-16 ***
    ---
    Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    
    Residual standard error: 0.01824 on 22951 degrees of freedom
      (75 observations deleted due to missingness)
    Multiple R-squared:  0.1716,	Adjusted R-squared:  0.1716 
    F-statistic:  4755 on 1 and 22951 DF,  p-value: < 2.2e-16
    


### Results:
- Better adjusted R-squared than the economic correctedness.
- p value is still significant.
- Coefficient value is negetive but still low.

## Civic Engagement :
### Rates of volunteering and participation in community organizations

> ### Simple regression with lm (dependant - Volunteering rates, Independant - number of establishment)


```R
civic.fit = lm(volunteering_rate_zip ~ESTAB, data=dataset)
summary(civic.fit)
```
    ...
    Call:
    lm(formula = volunteering_rate_zip ~ ESTAB, data = dataset)
    
    Residuals:
         Min       1Q   Median       3Q      Max 
    -0.07865 -0.02395 -0.00466  0.01719  0.48373 
    
    Coefficients:
                  Estimate Std. Error t value Pr(>|t|)    
    (Intercept)  8.124e-02  2.964e-04  274.07   <2e-16 ***
    ESTAB       -6.724e-06  2.615e-07  -25.71   <2e-16 ***
    ---
    Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    
    Residual standard error: 0.03635 on 22948 degrees of freedom
      (78 observations deleted due to missingness)
    Multiple R-squared:  0.028,	Adjusted R-squared:  0.02796 
    F-statistic: 661.1 on 1 and 22948 DF,  p-value: < 2.2e-16
    


### Results:
- Adjusted R-squared value is very low.
- p value is still significant.
- Coefficient value is negetive but still low.

> ### Simple regression with lm (dependant - Civic Organizations , Independant - number of establishment)


```R
org.fit = lm(civic_organizations_zip  ~ESTAB, data=dataset)
summary(org.fit)
```
    ...
    Call:
    lm(formula = civic_organizations_zip ~ ESTAB, data = dataset)
    
    Residuals:
         Min       1Q   Median       3Q      Max 
    -0.02915 -0.00993 -0.00450  0.00299  1.21537 
    
    Coefficients:
                 Estimate Std. Error t value Pr(>|t|)    
    (Intercept) 1.809e-02  2.500e-04  72.349  < 2e-16 ***
    ESTAB       1.431e-06  2.189e-07   6.541 6.25e-11 ***
    ---
    Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    
    Residual standard error: 0.03031 on 22599 degrees of freedom
      (427 observations deleted due to missingness)
    Multiple R-squared:  0.00189,	Adjusted R-squared:  0.001845 
    F-statistic: 42.78 on 1 and 22599 DF,  p-value: 6.246e-11
    


### Results:
- Adjusted R-squared value is lowest out of all.
- p value is still significant.
- Coefficient value is still low.
